...
#if (defined(_SOLARIS) || defined(_AIX) || defined(__LINUX__)) || defined (__FreeBSD__)
		    directio_on = 1;
#else
...
#if (defined(__LINUX__) || defined(_AIX)) || defined(__FreeBSD__)
		if (directio_on) {
		    flags |= O_DIRECT;
		}
#endif
...
work_thread (void *arg)
{
	...
	#ifdef __LINUX__
	// MDW: Add in alignment for raw io.
	malloc_size = blk_sz + LINUX_RAWIO_ALIGNMENT;
	...
	#ifdef __FreeBSD__
	malloc_size = blk_sz;
	alloc_memory = aligned_alloc (4096, blk_sz);
	buf = alloc_memory;
	#else
	malloc_size = blk_sz;
	...
	fprintf(stderr, "buf size = %d, non_zero_cnt = %d\n", blk_sz, non_zero_cnt);
